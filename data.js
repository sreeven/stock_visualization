...
d3.json("stock.json").then(function(data) {
    ...
    var sectors = [];
    var symbols = [];
    var symbols_per_sector = {};

    for (var i = 0; i < size; i++){
        if (sectors.includes(data[i].Sector) === false){
            sectors.push(data[i].Sector)
        }
    ...
    // var market_cap_per_sector = {};



    // for (var i = 0; i < sectors.size; i++){
    //     market_cap_per_sector.push({
    //         key: sectors[i],
    //         value: 1
    //     })
    // }

    // console.log(market_cap_per_sector)
